...
class ScheduleService:
    ...
    def _rule_dates(self,pid:int,start:date,end:date)->set[str]:
        ...
        for rule in self.database.closure_rules(pid):
            rs=date.fromisoformat(rule["start_date"]) if rule.get("start_date") else start
            re=date.fromisoformat(rule["end_date"]) if rule.get("end_date") else end
            rs=max(rs,start); re=min(re,end); current=rs
            while current<=re:
                ...
                if nth:
                    ...
                    if occurrence!=int(nth): current+=timedelta(days=1); continue
    ...
    def preview(self,profile="school",*,extra_blocked:set[str]|None=None)->tuple[list[dict[str,Any]],ScheduleSummary]:
        ...
        blocked=self.database.profile_blocked(pid)|self._rule_dates(pid,start,end)|(extra_blocked or set())
        holidays=self.database.profile_holiday_map(pid); overrides=self.database.overrides(pid)
        rows=[]; counts={"school":0,"non_school":0,"weekend":0}; current=start
        while current<=end:
            iso=current.isoformat(); override=overrides.get(iso); overridden=0
            if override and override["override_type"]=="no_school":
                kind="no_school"; cday=None; title=override.get("title") or "No School"; detail=override.get("note") or "Override"; source="override"; overridden=1
            elif override and override["override_type"]=="school":
                cday=int(override.get("cycle_day") or cycle); cday=((cday-1)%len(labels))+1; kind="school"; title=override.get("title") or f"Day {cday}"; detail=override.get("note") or labels[cday-1]; source="override"; overridden=1; cycle=(cday%len(labels))+1
            elif current.weekday()>=5:
                kind="weekend"; cday=None; title="Weekend"; detail=""; source="generated"
            elif iso in blocked:
                kind="no_school"; cday=None; title="No School"; detail=holidays.get(iso,"No School"); source="holiday" if iso in holidays else "non_school_day"
            else:
                kind="school"; cday=cycle; title=f"Day {cycle}"; detail=labels[cycle-1]; source="generated"; cycle=(cycle%len(labels))+1
    ...
    def rebuild_profile(self,profile="school")->ScheduleSummary:
        p=self._profile(profile); rows,summary=self.preview(profile); self.database.replace_profile_schedule(p["id"],rows); self.database.audit(p["id"],"schedule_rebuilt",summary.__dict__); return summary
